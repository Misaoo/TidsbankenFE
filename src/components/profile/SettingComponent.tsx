...
const SettingComponent = (props: any) => {
    ...
    let [passwdMsg2, setPasswdMsg2] = useState(""); // bool that determains which buttons to show in the update password modal
        ...
        if (await oldPassword === true && newPassword === true) {
            try {
                ...
                if (error.response.status === 401 || error.response.status === 504) {
                    ...
                    setNewPass1("")
                    setNewPass2("")
                    setPasswdMsg2('Failed to update password')
                    setTimeout(() => setPasswdMsg2(""), 3000)
                }
    ...
    const testOldPassword = async () => {
        try {
            ...
            if (error.response.status === 400) {
                ...
                setNewPass1("")
                setNewPass2("")
                setPasswdMsg2('Old password wrong.')
                setTimeout(() => setPasswdMsg2(""), 3000)
            }
        }
    }
    ...
    function testNewPasswords() {
        if (newPass1 === newPass2) {
            ...
            setNewPass1("")
            setNewPass2("")
            setPasswdMsg2('Passwords dont match.') 
            setTimeout(() => setPasswdMsg2(""), 3000)
            return false; 
        }
    }
    ...
    return (
        <div className={SettingsStyles.wrapper}>
            ...
            <Modal display={showModal} setDisplay={setshowModal} title="New password">
                <form key="updatePasswdForm" onSubmit={handlePasswordSubmit}>
                    <label key="oldPass" className={commonStyles.label} htmlFor="OldPassword">Old Password</label>
                    <input key="oldPassInpt"className={commonStyles.input} type="password" name="OldPassword" placeholder="Enter your old password" value={oldPass} onChange={handleChangeOldPass} required/>

                    <label key="newPass"className={commonStyles.label} htmlFor="NewPassword">New Password</label>
                    <input key="newPassInpt"className={commonStyles.input} type="password" name="NewPassword" placeholder="Enter your new password" value={newPass1} onChange={handleChangeNewPass1} required/>

                    <label key="newPassVer"className={commonStyles.label} htmlFor="NewPassword">New Password</label>
                    <input key="newPassVerInp"className={commonStyles.input} type="password" name="NewPassword" placeholder="Enter your new password" value={newPass2} onChange={handleChangeNewPass2} required/>
                ...
                {passwdMsg && (
                   <div className={[commonStyles.successLabel, commonStyles.buttonplacement].join(" ")}>{ passwdMsg }<br /></div>
                )}
                {passwdMsg2 && (
                   <div className={[commonStyles.badLabel, commonStyles.buttonplacement].join(" ")}>{ passwdMsg2 }<br /></div>
                )}
            </Modal>
