  ...
  const timer = () => {
    let timer: number = window.setInterval(() => {
      ...
      if (timeDelta < 0) {
        ...
        setMessage(
          `5 attemts made please try again in ${ls.minutesLeft} min and ${ls.secondsLeft} sec`
        );
      }
    }, 1000);
  };
  ...
  const handleSubmit = async (event: any) => {
    ...
    try {
      ...
      if (error.response.status === 401 || error.response.status === 504) {
        ...
        if (
          errorData.hasOwnProperty("timeOut") ||
          errorData["numOfAttemptedLogins"] === 5
        ) {
          setMessage(
            `5 attemts made please try again in ${ls.minutesLeft} min and ${ls.secondsLeft} sec`
          );
          ...
          setMessage(
            `You have ${5 - errorData["numOfAttemptedLogins"]} attempts left`
          );
        }
      }
